...
export function buildSuperHTML(items: ContentTableItem[]): string {
  ...
  const listItems = filtered
    .map(({ title, anchor, filters: itemFilters }) => {
      const liStyle = hasActiveItemFilters(itemFilters)
        ...
        : ''
      return `<li${liStyle}><a alias="${title}" conversion="false" data-linkto="anchortotext" href="${anchor}" style="color:#1895D3;text-decoration:underline;font-family:Verdana,Geneva,sans-serif;font-size:14px;font-style:normal;line-height:1.4" title="${title}">${title}</a></li>`
    })
    .join('\n')
...
export function buildEmailHTML(items: ContentTableItem[]): string {
  ...
  const listItems = filtered
    .map(({ title, anchor, filters: itemFilters }) => {
      const li = `<li><a alias="${title}" conversion="false" data-linkto="anchortotext" href="${anchor}" style="color:#1895D3;text-decoration:underline;font-family:Verdana,Geneva,sans-serif;font-size:14px;font-style:normal;line-height:1.4" title="${title}">${title}</a></li>`
